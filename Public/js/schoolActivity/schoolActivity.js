...
require(['jquery','complex','unslider'],function($,complex){
	...
	//$("#city").citySelect({url:'',prov:'四川省', city:'成都', dist:'青羊区'});
	...
	$('.advertise_type .typeDetail').find('span').bind('click',function(){
		$(this).addClass('getActive').siblings().removeClass('getActive');
		getVariable();
	});
	...
	function getVariable(){
		var $province=$('#province').val(),
			...
			$advertiseType=$('.advertise_type .typeDetail').find('.getActive').attr('name'),
		...
		data.advertise_id=$advertiseType?$advertiseType:0;//赞助方式
